...
rule all:
    input:
        ...
        expand(output_path + '/output/expression/sces/sce_{id}.rds', id=ids),
        'output/expression/sce.rds',
        #expand(output + 'pileup_phase/{id}_allele_counts.tsv.gz',
        #id=ids)
...
rule subset_vcf2:
    input:
        ...
        chrmap=config['chrmap'],
    output:
        output_path + '/vcf/genome1K_chr6.vcf'
    shell:
        'bcftools view '
        '{input.vcf} '
...
rule cellsnp:
    ...
    input:
        vcf = output_path + '/vcf/genome1K_chr6.vcf',
